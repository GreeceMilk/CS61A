...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    l = list(filter(select, paragraphs))
    if k >= len(l):
        return ''
    else:
        return l[k]
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def iscontain(p):
        pp = split(remove_punctuation(lower(p)))
        for word in subject:
            if word in pp:
                return True
        return False
...
def accuracy(typed, source):
    ...
    if len(typed_words)==0 and len(source_words)==0:
        ...
        return 0.0
    def accur_lst(typed_lst, source_lst):
        if typed_lst == [] or source_lst == []:
            return 0
        elif typed_lst[0] == source_lst[0]:
            return 100 + accur_lst(typed_lst[1:], source_lst[1:])
        else:
            return accur_lst(typed_lst[1:], source_lst[1:])
    return accur_lst(typed_words, source_words) / len(typed_words)
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    result_word = word_list[0]
    for word in word_list:
        if typed_word == word:
            return word
        elif diff_function(typed_word, word, limit) < diff_function(typed_word, result_word, limit):
            result_word = word
    return typed_word if diff_function(typed_word, result_word, limit) > limit else result_word
...
def report_progress(typed, source, user_id, upload):
    ...
    """
    # BEGIN PROBLEM 8
    def cal_progress(t, s):
        if len(t) == 0:
            return 0
        elif t[0] != s[0]:
            return 0
        else:
            return 1 + cal_progress(t[1:], s[1:])
    progress = cal_progress(typed, source) / len(source)
...
def time_per_word(words, timestamps_per_player):
    ...
    def get_time(timestamp):
        ...
        for i in range(1, len(timestamp)):
            lst.append(timestamp[i] - timestamp[i-1])
        return lst
    return match(words, [get_time(timestamps_per_player[0]), get_time(timestamps_per_player[1])])
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for p in player_indices:
            if time(match, p, i) < time(match, fastest_player, i):
                fastest_player = p
